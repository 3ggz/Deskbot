...
// Touch_CST820 intentionally omitted — Task 8 is display-only
...
void ST7701_Init()
{
  ...
  // RGB panel — using Arduino-ESP32 2.x esp_lcd_rgb_panel_config_t layout.
  // Fields like bits_per_pixel, num_fbs, bounce_buffer_size_px, double_fb
  // do not exist in 2.x and are omitted.
...
// LCD_Init: calls ST7701_Reset + ST7701_Init + Backlight_Init.
// Touch_Init() is intentionally NOT called here — Task 8 is display-only.
void LCD_Init() {
  ST7701_Reset();
  ST7701_Init();
  Backlight_Init();
...
void Backlight_Init()
{
  ...
  // (ledcAttach() is Arduino-ESP32 3.x only)
